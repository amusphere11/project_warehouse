  ...
  const handleSubmit = async () => {
    try {
      if (editMode && selectedMaterial) {
        await api.put(`/api/materials/${selectedMaterial.id}`, formData);
      } else {
  ...
  return (
    <Box>
      <Box display="flex" justifyContent="space-between" alignItems="center" mb={3}>
        <Typography variant="h4">Materials Master Data</Typography>
        <Button
          variant="contained"
          startIcon={<Add />}
      ...
      <Dialog open={openDialog} onClose={handleCloseDialog} maxWidth="sm" fullWidth>
        <DialogTitle>{editMode ? 'Edit Material' : 'Add New Material'}</DialogTitle>
        <DialogContent>
          <TextField
            label="Barcode"
            fullWidth
